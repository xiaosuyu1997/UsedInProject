...
### just for test
#loginfo("Comparison:")
#Y.cor.limma <- removeBatchEffect(Y,myDesign$patient)
#range(Y.cor.limma)
#print(Y.cor.limma[1:4,1:8])
#
#Y.cor.my.keepT <- t(my.rmBatchEffect(t(Y), myDesign$patient, keep.intercept=T))
#range(Y.cor.my.keepT)
#print(Y.cor.my.keepT[1:4,1:8])
#
#all((Y.cor.my.keepT-Y.cor.limma)<10e-10)
#
#Y.cor.my.keepF <- t(my.rmBatchEffect(t(Y), myDesign$patient, keep.intercept=F))
#range(Y.cor.my.keepF)
#print(Y.cor.my.keepF[1:4,1:8])
#
#Y.centered <- my.centerData(Y, myDesign)
#range(Y.centered)
#print(Y.centered[1:4,1:8])
#
#all((Y.cor.my.keepF-Y.centered)<10e-10)
#
#lenv <- loadToEnv(inputFile)
#
#Y.test <- exprs(lenv$sce.norm)
#Y.test.un <- norm_exprs(lenv$sce.norm)
#.test.d <- pData(lenv$sce.norm)
#Y.test[1:4,1:8]
#Y.test.un[1:4,1:8]
#Y.test.un.cor[1:4,1:8]
#Y.test.un.cor <- my.centerData(Y.test.un,.test.d)
#Y.test.un.cor <- Y.test.un.cor[rownames(Y.test),colnames(Y.test)]
#all(rownames(Y.test)==rownames(Y.test.un.cor))
#
#Y.test <- Y.test[rownames(Y.centered),colnames(Y.centered)]
#range(Y.test)
#print(Y.test[1:4,1:8])
#all((Y.test-Y.centered)<10e-10)
