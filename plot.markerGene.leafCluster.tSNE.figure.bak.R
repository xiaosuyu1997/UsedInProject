...
do.plot.integratedFigure <- function(newick.file=NULL){
    ...
    layout(matrix(c(1,2,3,1,4,4),ncol=3,byrow = T),widths = c(0.5,0.2,0.3),heights = c(0.75,0.25))
    par(mar=c(5,5,4,2),cex.lab=1.5,cex.main=1.5)
    plot(tsne.out[["30"]]$Rtsne.res$Y[,1],tsne.out[["30"]]$Rtsne.res$Y[,2], t='p',pch=16,col="lightgray", main="BarnesHutSNE",xlab="Dim1",ylab="Dim2")
    ...
    par(mar=c(0,2,4,0),cex.lab=1.5,cex.main=1.5)
    ...
    par(mar=c(0,1,4,9),cex.lab=1.5,cex.main=1.5)
    ...
    ### legend
    par(mar=c(1,0,1,1),cex.lab=1.5,cex.main=1.5)
    plot.new()
    ##idx.leaf <- ceiling(length(colnames(dat.plot))/2)
    print(colnames(dat.plot))
    idx.range <- 1:4
    legend("topleft", sapply(colnames(dat.plot)[idx.range],function(x){ 
                         x  %>% gsub("_"," ",.) %>% strwrap(20) %>% paste(.,collapse="\n") }),
           col=leafClusterColor[colnames(dat.plot)[idx.range]],
           cex=1.2,pt.cex=4,pch=16,xpd=NA,inset=c(0.00,0.15),horiz = T,bty="n",yjust = 0)
    idx.range <- 5:min(8,ncol(dat.plot))
    legend("left",sapply(colnames(dat.plot)[idx.range],function(x){ 
                        x  %>% gsub("_"," ",.) %>% strwrap(20) %>% paste(.,collapse="\n") }),
           col=leafClusterColor[colnames(dat.plot)[idx.range]],
           cex=1.2,pt.cex=4,pch=16,xpd=NA,inset=c(0.00,0),horiz = T,bty="n",yjust = 0.5)
    if(ncol(dat.plot)>=9){
        idx.range <- 9:ncol(dat.plot)
        legend("bottomleft",sapply(colnames(dat.plot)[idx.range],function(x){ 
                            x  %>% gsub("_"," ",.) %>% strwrap(20) %>% paste(.,collapse="\n") }),
               col=leafClusterColor[colnames(dat.plot)[idx.range]],
               cex=1.2,pt.cex=4,pch=16,xpd=NA,inset=c(0.00,0.15),horiz = T,bty="n",yjust = 1)
    }
    dev.off()
}
