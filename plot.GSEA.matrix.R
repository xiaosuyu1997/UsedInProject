...
#GSEA.list.pos.file <- "/WPSnew/zhenglt/work/proj_zl/integrate0924/cross.patient.b20170926/functional/ssGSEA/zscore/table.pos.list"
#GSEA.list.neg.file <- "/WPSnew/zhenglt/work/proj_zl/integrate0924/cross.patient.b20170926/functional/ssGSEA/zscore/table.neg.list"
#clusterColorFile <- "/WPSnew/zhenglt/work/proj_zl/integrate0924/cross.patient.b20170926/majorClusterColor.mod.txt"
#out.prefix <- "/WPSnew/zhenglt/work/proj_zl/integrate0924/cross.patient.b20170926/functional/ssGSEA/zscore/colonC.cluster.GSEA"
#args.FDR <- 0.01
...
###############
###for(mcls in colnames(dat.plot.acast.FDR)){
###    f.gset <- dat.plot.acast.FDR[,mcls]<0.01

for(mcls in unique(dat.plot$majorCluster))
{
    print(mcls)
    sigScore.max <- 10
    ##f.gset <- dat.plot[,"FDR q-val"]<0.01 & dat.plot[,"majorCluster"]==mcls
    f.gset <- dat.plot[,"FDR q-val"]<0.01 & dat.plot[,"NES"]>0 & dat.plot[,"majorCluster"]==mcls
    dat.plot.mcls <- dat.plot[f.gset,]
    colnames(dat.plot.mcls) <- make.names(colnames(dat.plot.mcls))
    dat.plot.mcls <- dat.plot.mcls[order(dat.plot.mcls$FDR.q.val,-abs(dat.plot.mcls$NES)),] 
    dat.plot.mcls <- head(dat.plot.mcls,n=20)
    dat.plot.mcls$geneSet <- dat.plot.mcls$NAME
    f.longname <- nchar(dat.plot.mcls$geneSet)>32
    dat.plot.mcls$geneSet <- strtrim(dat.plot.mcls$geneSet,32)
    dat.plot.mcls$geneSet[f.longname] <- sprintf("%s...",dat.plot.mcls$geneSet[f.longname])
    dat.plot.mcls$geneSet <- sprintf("%s(%d)",dat.plot.mcls$geneSet,dat.plot.mcls$SIZE)
    idx <- order(dat.plot.mcls$NES, decreasing = F)
    dat.plot.mcls$geneSet <- factor(dat.plot.mcls$geneSet, levels=dat.plot.mcls$geneSet[idx])
    dat.plot.mcls$sigScore <- -log10(dat.plot.mcls$FDR.q.val)/2
    dat.plot.mcls$sigScore[dat.plot.mcls$sigScore>sigScore.max] <- sigScore.max
    p <- ggplot(dat.plot.mcls,aes(x=NES, y=geneSet)) +
        geom_point(aes(size=sigScore,colour=sigScore)) +
        ylab("") + xlab("NES") +
        scale_color_gradientn(colours=brewer.pal(7,"YlOrRd"),limits=c(-log10(1),sigScore.max)) +
        theme_bw() +
    ...
    ggsave(filename = sprintf("%s.%s.dotplot.pdf",out.prefix,mcls),width = 6,height = max(0.2*nrow(dat.plot.mcls),6))
}
