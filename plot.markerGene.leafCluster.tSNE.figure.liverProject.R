...
#if(isTest){
#    lname <- load("/WPS1/zhenglt/work/TCR_chunhong/integrated.20150707/hetero/cross.patient/new.clusteringResult/liver.tSNE.colorByCluster.useAOV.n1500.obj.RData")
#    print(lname)
#    lname <- load("/WPS1/zhenglt/work/TCR_chunhong/integrated.20150707/hetero/cross.patient/new.clusteringResult/liver.tSNE.colorByCluster.useAOV.n1500.liver.Y.RData")
#}
...
do.plot.integratedFigure <- function(newick.file=NULL,perplexity=30,CD8CD4=F){
    ...
    for(m.cls in c("CD8_Cluster1-LEF1","CD8_Cluster5-GZMK","CD8_Cluster3-SLC4A10","CD8_Cluster2-CX3CR1","CD8_Cluster4-LAYN", "CD4_Cluster2-IL2RA","CD4_Cluster1-CCR7","CD4_Cluster4-GZMA","CD4_Cluster3-CTLA4","CD4_Cluster5-CXCL13","CD4_Cluster6-GNLY")){
        f.points <- sample.desc$majorCluster==m.cls
    ...
    dat.plot <- dat.plot[,labels(hc.merged)]
    par(mar=c(6,16,4,6),cex.lab=1.5,cex.main=1.5)
    ...
    staxlab(2,at = xx,labels=colnames(dat.plot),srt=0, cex=1.5,adj=1,top.line=0.5)
    ...
    tmp.cluster.levels <- c("CD8_Cluster4-LAYN", "CD8_Cluster5-GZMK","CD8_Cluster3-SLC4A10", "CD8_Cluster2-CX3CR1","CD8_Cluster1-LEF1",
                            "CD4_Cluster6-GNLY","CD4_Cluster4-GZMA","CD4_Cluster5-CXCL13","CD4_Cluster1-CCR7","CD4_Cluster2-IL2RA","CD4_Cluster3-CTLA4")
    ...
    dat.plot <- s.table[grepl("^TTC",s.table$sampleType,perl=T),,drop=F]
    if(nrow(dat.plot)>0){
        ...
        cat("382 line here\n")
        write.table(dat.plot.df,sprintf("%s.integrated.perplexity%d.CD8.T.subtypeDist.txt",out.prefix,perplexity),quote = F,sep = "\t",row.names = F)

        pdf(sprintf("%s.integrated.perplexity%d.CD8.T.subtypeDist.pdf",out.prefix,perplexity),width=6,height=10)
        par(mar=c(5,6,12,2),cex.lab=2,cex.main=2,cex.axis=1.5)
        ...
        title(main="CD8+ T cell (T)",line=1)
        legend("top",legend=rownames(dat.plot),fill=majorClusterColor[rownames(dat.plot)],inset = c(0,-0.35),xpd = T,horiz = F,cex=1.2)
        dev.off()
    }

    #dat.plot <- s.table[grepl("^NTC",s.table$sampleType,perl=T) & s.table$majorCluster!="CD8_naive",,drop=F]
    dat.plot <- s.table[grepl("^NTC",s.table$sampleType,perl=T),,drop=F]
    if(nrow(dat.plot)>0){
        dat.plot <- acast(dat.plot,majorCluster~patient,value.var="sample")
        dat.plot[is.na(dat.plot)] <- 0
        ##apply(dat.plot,2,sum)
        ...
        dat.plot.df <- data.frame(subtype=rownames(dat.plot))
        dat.plot.df <- cbind(dat.plot.df,dat.plot)
        write.table(dat.plot.df,sprintf("%s.integrated.perplexity%d.CD8.N.subtypeDist.txt",out.prefix,perplexity),quote = F,sep = "\t",row.names = F)

        pdf(sprintf("%s.integrated.perplexity%d.CD8.N.subtypeDist.pdf",out.prefix,perplexity),width=6,height=10)
        par(mar=c(5,6,12,2),cex.lab=2,cex.main=2,cex.axis=1.5)
        xx <- barplot(dat.plot,beside = F,xlab="",ylab="Cell Subtype",main="",col=majorClusterColor[rownames(dat.plot)],xaxt="n")
        #staxlab(1,at = xx,labels=names(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3,adj=0.5,top.line=2)
        staxlab(1,at = xx,labels=colnames(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3)
        title(main="CD8+ T cell (N)",line=1)
        legend("top",legend=rownames(dat.plot),fill=majorClusterColor[rownames(dat.plot)],inset = c(0,-0.35),xpd = T,horiz = F,cex=1.2)
        dev.off()
    }
    
    dat.plot <- s.table[grepl("^PTC",s.table$sampleType,perl=T),,drop=F]
    if(nrow(dat.plot)>0){
        dat.plot <- acast(dat.plot,majorCluster~patient,value.var="sample")
        dat.plot[is.na(dat.plot)] <- 0
        ##apply(dat.plot,2,sum)
        ...
        dat.plot.df <- data.frame(subtype=rownames(dat.plot))
        dat.plot.df <- cbind(dat.plot.df,dat.plot)
        write.table(dat.plot.df,sprintf("%s.integrated.perplexity%d.CD8.P.subtypeDist.txt",out.prefix,perplexity),quote = F,sep = "\t",row.names = F)

        pdf(sprintf("%s.integrated.perplexity%d.CD8.P.subtypeDist.pdf",out.prefix,perplexity),width=6,height=10)
        par(mar=c(5,6,12,2),cex.lab=2,cex.main=2,cex.axis=1.5)
        xx <- barplot(dat.plot,beside = F,xlab="",ylab="Cell Subtype",main="",col=majorClusterColor[rownames(dat.plot)],xaxt="n")
        #staxlab(1,at = xx,labels=names(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3,adj=0.5,top.line=2)
        staxlab(1,at = xx,labels=colnames(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3)
        title(main="CD8+ T cell (P)",line=1)
        legend("top",legend=rownames(dat.plot),fill=majorClusterColor[rownames(dat.plot)],inset = c(0,-0.35),xpd = T,horiz = F,cex=1.2)
        dev.off()
    }
    
    #dat.plot <- s.table[grepl("^(TTH|TTR)",s.table$sampleType,perl=T) & !grepl("^CD4_P",s.table$majorCluster,perl=T),,drop=F]
    dat.plot <- s.table[grepl("^(TTH|TTR)",s.table$sampleType,perl=T) ,,drop=F]
    if(nrow(dat.plot)>0){
        ...
        dat.plot.df <- data.frame(subtype=rownames(dat.plot))
        dat.plot.df <- cbind(dat.plot.df,dat.plot)
        write.table(dat.plot.df,sprintf("%s.integrated.perplexity%d.CD4.T.subtypeDist.txt",out.prefix,perplexity),quote = F,sep = "\t",row.names = F)

        pdf(sprintf("%s.integrated.perplexity%d.CD4.T.subtypeDist.pdf",out.prefix,perplexity),width=6,height=10)
        par(mar=c(5,6,12,2),cex.lab=2,cex.main=2,cex.axis=1.5)
        xx <- barplot(dat.plot,beside = F,xlab="",ylab="Cell Subtype",main="",col=majorClusterColor[rownames(dat.plot)],xaxt="n")
        #staxlab(1,at = xx,labels=names(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3,adj=0.5,top.line=2)
        staxlab(1,at = xx,labels=colnames(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3)
        title(main="CD4+ T cell (T)",line=1)
        legend("top",legend=rownames(dat.plot),fill=majorClusterColor[rownames(dat.plot)],inset = c(0,-0.35),xpd = T,horiz = F,cex=1.2)
        dev.off()
    }

    #dat.plot <- s.table[grepl("^(NTH|NTR)",s.table$sampleType,perl=T) & !grepl("^CD4_P",s.table$majorCluster,perl=T),,drop=F]
    dat.plot <- s.table[grepl("^(NTH|NTR)",s.table$sampleType,perl=T) ,,drop=F]
    if(nrow(dat.plot)>0){
        dat.plot <- acast(dat.plot,majorCluster~patient,value.var="sample",fun.aggregate=sum)
        dat.plot[is.na(dat.plot)] <- 0
        ##apply(dat.plot,2,sum)
        ...
        dat.plot.df <- data.frame(subtype=rownames(dat.plot))
        dat.plot.df <- cbind(dat.plot.df,dat.plot)
        write.table(dat.plot.df,sprintf("%s.integrated.perplexity%d.CD4.N.subtypeDist.txt",out.prefix,perplexity),quote = F,sep = "\t",row.names = F)

        pdf(sprintf("%s.integrated.perplexity%d.CD4.N.subtypeDist.pdf",out.prefix,perplexity),width=6,height=10)
        par(mar=c(5,6,12,2),cex.lab=2,cex.main=2,cex.axis=1.5)
        xx <- barplot(dat.plot,beside = F,xlab="",ylab="Cell Subtype",main="",col=majorClusterColor[rownames(dat.plot)],xaxt="n")
        #staxlab(1,at = xx,labels=names(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3,adj=0.5,top.line=2)
        staxlab(1,at = xx,labels=colnames(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3)
        title(main="CD4+ T cell (N)",line=1)
        legend("top",legend=rownames(dat.plot),fill=majorClusterColor[rownames(dat.plot)],inset = c(0,-0.35),xpd = T,horiz = F,cex=1.2)
        dev.off()
    }
    
    dat.plot <- s.table[grepl("^(PTH|PTR)",s.table$sampleType,perl=T) ,,drop=F]
    if(nrow(dat.plot)>0){
        dat.plot <- acast(dat.plot,majorCluster~patient,value.var="sample",fun.aggregate=sum)
        dat.plot[is.na(dat.plot)] <- 0
        ##apply(dat.plot,2,sum)
        ...
        dat.plot.df <- data.frame(subtype=rownames(dat.plot))
        dat.plot.df <- cbind(dat.plot.df,dat.plot)
        write.table(dat.plot.df,sprintf("%s.integrated.perplexity%d.CD4.P.subtypeDist.txt",out.prefix,perplexity),quote = F,sep = "\t",row.names = F)

        pdf(sprintf("%s.integrated.perplexity%d.CD4.P.subtypeDist.pdf",out.prefix,perplexity),width=6,height=10)
        par(mar=c(5,6,12,2),cex.lab=2,cex.main=2,cex.axis=1.5)
        xx <- barplot(dat.plot,beside = F,xlab="",ylab="Cell Subtype",main="",col=majorClusterColor[rownames(dat.plot)],xaxt="n")
        #staxlab(1,at = xx,labels=names(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3,adj=0.5,top.line=2)
        staxlab(1,at = xx,labels=colnames(dat.plot),srt=if(length(dat.plot) >= 5) 45 else 0, cex=1.3)
        title(main="CD4+ T cell (P)",line=1)
        legend("top",legend=rownames(dat.plot),fill=majorClusterColor[rownames(dat.plot)],inset = c(0,-0.35),xpd = T,horiz = F,cex=1.2)
        dev.off()
    }
