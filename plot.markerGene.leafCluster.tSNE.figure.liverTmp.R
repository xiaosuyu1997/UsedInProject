...
if(!is.null(args.preResult) && file.exists(args.preResult)){
    ...
    sample.desc <- sample.desc[rownames(penv[["sample.desc"]]),]
}else{
    ...
    tsne.dbscan.out <- runTSNEAnalysis(Y[g.f,],sprintf("%s.dbScan",out.prefix),
                                       ...
                                       pch=16,pch.legend=16,
                                       inPDF=TRUE,eps=1.20,dims=2,k=NULL,
                                       do.dbscan=T,myseed=rn.seed,preSNE=tsne.out)

    loginfo("tSNE done.")
}

sample.desc$tmp.sampleType <- paste(sapply(strsplit(sample.desc$majorCluster,""),function(x){ paste(x[1],x[2],x[3],sep="")}),
                                    sapply(strsplit(sample.desc$sampleType,""),function(x){x[1]}),sep=".")
print(head(sample.desc))
print(str(sample.desc))
...
do.plot.integratedFigure <- function(newick.file=NULL,perplexity=30,CD8CD4=F){
    ...
    plot(tsne.out[[as.character(perplexity)]]$Rtsne.res$Y, 
         ...
         cex=min(1.0,0.8*7000/nrow(tsne.out[[as.character(perplexity)]]$Rtsne.res$Y)))
    ### background + contour
    plot(tsne.out[[as.character(perplexity)]]$Rtsne.res$Y, 
         t='p',pch=16,col="lightgray", main="BarnesHutSNE",xlab="Dim1",ylab="Dim2",cex.lab=2.0,cex.axis=1.5,cex.main=2.0,
         cex=min(1.0,0.8*7000/nrow(tsne.out[[as.character(perplexity)]]$Rtsne.res$Y)))
    ...
    s.table <- aggregate(sample~patient+tmp.sampleType+majorCluster,data=sample.desc,FUN=length)
    ...
    #                        "MAIT","iNKT")
    tmp.cluster.levels <- rev(c("CD8_Cluster1-LEF1","CD8_Cluster2-CX3CR1","CD8_Cluster3-SLC4A10","CD8_Cluster5-GZMK","CD8_Cluster4-LAYN",
                            "CD4_Cluster6-GNLY","CD4_Cluster1-CCR7","CD4_Cluster2-IL2RA","CD4_Cluster3-CTLA4","CD4_Cluster4-GZMA","CD4_Cluster5-CXCL13"))
    ...
    for(.locType in unique(s.table$tmp.sampleType))
    {
        .locType.vec <- unlist(strsplit(.locType,"\\."))
        dat.plot <- subset(s.table,tmp.sampleType==.locType)
        if(nrow(dat.plot)>0){
