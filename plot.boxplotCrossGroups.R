...
parser$add_argument("-z", "--normExprs", action="store_true", default=FALSE, help="use norm_exprs assay [default %(default)s]")
parser$add_argument("-m", "--measure", type="character", help="measurement (norm_exprs, exprs, counts, tpm) [ %(default)s]")
...
g.inputD <- processInput(designFile = sample.desc.file,cellTypeColorFile = NULL ,inputFile = in.file,
                         args.notFilter=T,geneFile = NULL, args.center,args.log,args.norm.exprs = args.norm.exprs,args.measure=args.measure)
...
ylim.range <- pretty(as.matrix(dat.plot)[,,drop=T])
if(args.perGene){
    for(i in seq_along(gname.list)){
        ...
        if(args.plotType=="violin"){
            ...
            ### violin
            plot(0,0,type="n",xlim=c(0.5,length(uClus)+0.5), ylim=c(ylim.range[1],ylim.range[length(ylim.range)]), 
                 xaxt = 'n',yaxt="n", xlab ="",ylab="",main="",axes=0)
            par(bty="n")
            t.lo <- 0
            t.hi <- 14
            ##t.lo <- min(0,ylim.range[1])
            ##t.hi <- max(14,ylim.range[length(ylim.range)])
            col.base <- c("yellow","red","black")
            ###col.base <- auto.colSet(n=3,name="Reds")
            ###col.base <- auto.colSet(n=3,name="YlOrRd")
            for(jj in seq_along(uClus))
            {
                ...
                d.col <- rgb(colorRamp(col.base)((t.mean-t.lo)/(t.hi-t.lo))/255)
            ...
            rect(seq_along(uClus)-0.5,rep(0*wTrack+0.5,length(uClus)),seq_along(uClus)+0.5,rep(1*wTrack+0.5,length(uClus)),
                 col=majorClusterColor[gsub("\\..$","",uClus,perl=T)],border=NA)
            ...
            image(matrix(1:100),col=colorRampPalette(col.base)(100), ylab="",yaxt="n",xaxt="n")
            ##axis(1,seq(0,1,1/7),labels = seq(0,14,14/7),padj=1.2)
            ###axis(1,(ylim.range-ylim.range[1])/(ylim.range[length(ylim.range)]-ylim.range[1]),labels = ylim.range,padj=1.2)
            .t.range <- pretty(c(t.lo,t.hi))
            axis(1,seq(0,1,1/(length(.t.range)-1)),labels = .t.range,padj=1.2)
            dev.off()
    ...
    ##nMax=10
    loginfo(sprintf("nUsed: %d, nMax: %d, height: %d",nUsed,nMax, min(nUsed,nMax)*1+2))
    npdf <- ceiling(length(gid.list)/nMax)
    if(args.plotType=="violin"){
        for(ipdf in 1:npdf){
            pdf(sprintf("%s.violin.%04d.pdf",out.prefix,ipdf),width=pdf.width,height=min(nUsed,nMax)*1+2)
            ###pdf(sprintf("%s.violin.%04d.pdf",out.prefix,ipdf),width=pdf.width,height=nMax*1+2)
            ##layout(mat = matrix(seq_len(nUsed+2),ncol = 1,byrow = T),heights = c(1,rep(1,nUsed+1),8))
            layout(mat = matrix(seq_len(min(nUsed,nMax)+2),ncol = 1,byrow = T),heights = c(rep(1,min(nUsed,nMax)+1),1,8))
            ...
            t.hi <- 14
            col.base <- c("yellow","red","black")
            for(i in ((ipdf-1)*nMax+1):(min(ipdf*nMax,length(gid.list))) ){
                plot(0,0,type="n",xlim=c(0.5,length(uClus)+0.5), ylim=c(ylim.range[1],ylim.range[length(ylim.range)]), 
                     xaxt = 'n',yaxt="n", xlab ="",ylab="",main="",axes=0)
                par(bty="n")
                for(jj in seq_along(uClus))
