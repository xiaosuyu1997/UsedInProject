...
sub run
{
	...
	print $out_conv join("\t","CDR3_Beta","ncells",
        ...
        "nDNA","n_per_DNA","DNA",
        "n.convergent.alpha","convergent.alpha.aa","convergent.alpha.DNA")."\n";
	while (my $row = $next->())
	{
		...
		foreach my $_cdr3Beta_i (@beta_forClone){
            ...
            ### examine alpha
            if($productiveAlphaBeta4[0]=~/True/i && $cdr3AlphaBeta4[0]!~/Couldn/) { push @alpha_forClone,0; }
            if($productiveAlphaBeta4[1]=~/True/i && $cdr3AlphaBeta4[1]!~/Couldn/) { push @alpha_forClone,1; }
            foreach my $_cdr3Alpha_i (@alpha_forClone){
                $betaClone{$cdr3AlphaBeta4[$_cdr3Beta_i]}->{"alpha"}->{"aa"}->{$cdr3AlphaBeta4[$_cdr3Alpha_i]}->{$idAlphaBeta4[$_cdr3Alpha_i]}++;
            }
		}
	...
	for my $_beta (sort { $betaClone{$b}->{"count"}<=>$betaClone{$a}->{"count"} } keys %betaClone){
        ...
        my @_alphaAA=keys %{$betaClone{$_beta}->{"alpha"}->{"aa"}};
        my $_alphaConvergent=0;
        my @_alphaConvergent_AA=();
        my @_alphaConvergent_DNA=();
        foreach my $_alpha (@_alphaAA){
            my @_alpha_DNA=keys %{$betaClone{$_beta}->{"alpha"}->{"aa"}->{$_alpha}};
            if(@_alpha_DNA>1){
                $_alphaConvergent++;
                push @_alphaConvergent_AA,$_alpha;
                push @_alphaConvergent_DNA,join(",",@_alpha_DNA);
            }
        }
		...
		print $out_conv join("\t",$_beta,$betaClone{$_beta}->{"count"},
            ...
            $_alphaConvergent,
            join(";",@_alphaConvergent_AA),join(";",@_alphaConvergent_DNA)
        )."\n";
