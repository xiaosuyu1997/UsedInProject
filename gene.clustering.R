...
parser$add_argument("-t", "--bgFC", type="double", default=1.0, help="bgFC, used in bin method [default %(default)s]")
parser$add_argument("-q", "--thr.cor", type="double", default=0.01, help="threshold of correlation pvalue, used in bin method [default %(default)s]")
parser$add_argument("-z", "--nlevels", type="integer", default=3, help="nlevels of discretized exp, used in bin method [default %(default)s]")
...
args.bgFC <- args$bgFC
args.thr.cor <- args$thr.cor
args.nlevels <- args$nlevels
...
###sample.desc.file <- "/WPS1/zhenglt/work/proj_xy/integrated/cross.patient.rename/clusteringResult/lungC.all.leaf.addMajor.final.rename.Treg.forPlot.txt"
###gene.file <- NULL
###out.prefix <- "/WPS1/zhenglt/work/proj_xy/integrated/cross.patient.rename/test/Treg"
###args.bgFC <- -10
###args.nlevels <- 3
###sample.id <- "Treg"
...
if(grepl("\\.scran.+RData$",in.file,perl=T)){
    ...
    #lenv[["sce.norm"]] <- toSingleCellExperiment(lenv[["sce.norm"]])
...
annfile.list <- c(
                  #"GO.BP"="/DBS/DB_temp/zhangLab/MSigDB/msigdb_v6.0_files_to_download_locally/msigdb_v6.0_GMTs/c5.bp.v6.0.entrez.gmt",
                  "GO.BP"="/WPSnew/zhenglt/00.database/MSigDB/msigdb_v6.1_files_to_download_locally/msigdb_v6.1_GMTs/c5.bp.v6.1.entrez.gmt",
                  #"GO.MF"="/DBS/DB_temp/zhangLab/MSigDB/msigdb_v6.0_files_to_download_locally/msigdb_v6.0_GMTs/c5.mf.v6.0.entrez.gmt",
                  "GO.MF"="/WPSnew/zhenglt/00.database/MSigDB/msigdb_v6.1_files_to_download_locally/msigdb_v6.1_GMTs/c5.mf.v6.1.entrez.gmt",
                  #"immunoSig"="/DBS/DB_temp/zhangLab/MSigDB/msigdb_v6.0_files_to_download_locally/msigdb_v6.0_GMTs/c7.all.v6.0.entrez.gmt",
                  "immunoSig"="/WPSnew/zhenglt/00.database/MSigDB/msigdb_v6.1_files_to_download_locally/msigdb_v6.1_GMTs/c7.all.v6.1.entrez.gmt",
                  #"canonicalPathways"="/DBS/DB_temp/zhangLab/MSigDB/msigdb_v6.0_files_to_download_locally/msigdb_v6.0_GMTs/c2.cp.v6.0.entrez.gmt",
                  "canonicalPathways"="/WPSnew/zhenglt/00.database/MSigDB/msigdb_v6.1_files_to_download_locally/msigdb_v6.1_GMTs/c2.cp.v6.1.entrez.gmt",
                  #"positional"="/DBS/DB_temp/zhangLab/MSigDB/msigdb_v6.0_files_to_download_locally/msigdb_v6.0_GMTs/c1.all.v6.0.entrez.gmt",
                  #"positional"="/WPSnew/zhenglt/00.database/MSigDB/msigdb_v6.1_files_to_download_locally/msigdb_v6.1_GMTs/c1.all.v6.1.entrez.gmt",
                  #"motif"="/DBS/DB_temp/zhangLab/MSigDB/msigdb_v6.0_files_to_download_locally/msigdb_v6.0_GMTs/c3.all.v6.0.entrez.gmt",
                  "motif"="/WPSnew/zhenglt/00.database/MSigDB/msigdb_v6.1_files_to_download_locally/msigdb_v6.1_GMTs/c3.all.v6.1.entrez.gmt",
...
{
    if(file.exists(annfile.list[[v]])){
        cat(sprintf("read db %s from %s \n",v,annfile.list[[v]]))
...
plot.heatmapByComplexHeatmap <- function(dat.plot,gene.desc,out.prefix,majorClusterV,majorClusterColor,rowCluster,rowClusterColor,group.downsample=50,downsample=T,do.scale=F,z.lo=-2,z.hi=2,z.step=0.5)
{
    ...
    ha.row <- HeatmapAnnotation(df = data.frame(Group=rowCluster), col = list(Group=rowClusterColor), 
                                show_legend = F, annotation_legend_param = list(),which = "row")
...
run.clusMaxAndPlot <- function(out.prefix,dat.to.sort,col.sort=F,do.dynCut=F)
{
    ...
    gene.desc <- data.frame(geneID=rownames(dat.to.sort),stringsAsFactors = F)
    gene.desc$geneSymbol <- g.GNAME[gene.desc$geneID]
    gene.desc$Group <- sprintf("G%02d",gene.clusters)
    rownames(gene.desc) <- gene.desc$geneID
    ## heatmap
    dat.plot <- sortByGeneGroup(dat.to.sort,gene.desc,col.sort)
    ###clust.col <- rainbow(max(gene.clusters))
    ...
    names(clust.col) <- as.character(unique(gene.desc$Group))
    print(str(colnames(dat.plot)))
    #.mcl.name <- strsplit(colnames(dat.plot),split="\\.",perl=T) %>% sapply(.,function(x){x[1]})
    ##.mcl.name <- colnames(.mcl.name)
    .mcl.name <- colnames(dat.plot)
    names(.mcl.name) <- colnames(dat.plot)
...
get.discretized.pattern <- function(l,n)
{
    stopifnot(l <= 9)
    pp <- expand.grid(as.data.frame(matrix(rep(seq(l)-1,n),nrow=l)))
    ### to do: remove thoes redunct row vector
    ###
    rownames(pp) <- apply(pp,1,function(x){ sprintf("P%s",paste(x,collapse="")) })
    return(as.matrix(pp))
}

run.binarizedPatternAndPlot <- function(out.prefix,dat.to.sort,aov.res,thr.fc=1,thr.p=0.01,thr.bgFC=-10,thr.cor.p=0.01,useMean=T,col.sort=F,nl=3,n.cores=8)
{
    require("dynamicTreeCut")
    require("dendextend")
    ...
    .d.pattern <- get.discretized.pattern(nl,ncol(dat.to.sort))
    colnames(.d.pattern) <- colnames(dat.to.sort)

    registerDoParallel(cores = n.cores)
    dat.to.sort.pattern <- ldply(rownames(dat.to.sort),function(x.rname){
        ###x.rname <- "3003"
        x <- dat.to.sort[x.rname,]
        idx.ref <- which.min(x)
        name.ref <- names(x)[idx.ref]
        name.alt <- names(x)[-idx.ref]
        .i.pattern.res <- as.data.frame(t(sapply(seq_len(nrow(.d.pattern)),function(i){
                  res <- cor.test(x,.d.pattern[i,])
                  return(structure(c(res$estimate,res$p.value),names=c("pattern.cor","pattern.p")))
                    })))
        .i.pattern.res <- cbind(data.frame(Group=rownames(.d.pattern),stringsAsFactors = F),.i.pattern.res)
        rownames(.i.pattern.res) <- rownames(.d.pattern)

        .i.pattern.res <- .i.pattern.res[order(.i.pattern.res$pattern.p),]
        .i.pattern.res <- subset(.i.pattern.res,pattern.cor>0 & pattern.p<thr.cor.p)
        .i.pattern.res$confirmed.aov <- sapply(rownames(.i.pattern.res),function(.p.name){
            .the.pattern <- .d.pattern[.p.name,]
            .the.pattern <- .the.pattern[.the.pattern > 0]
            .cname1 <- sprintf("HSD.diff.%s-%s",names(.the.pattern),name.ref)
            .cname2 <- sprintf("HSD.padj.%s-%s",names(.the.pattern),name.ref)
            if(all(.cname1 %in% colnames(lookup.table))){
                all(lookup.table[x.rname,.cname1] > thr.fc*.the.pattern & 
                    lookup.table[x.rname,.cname2] < thr.p & 
                    dat.to.sort[x.rname,names(.the.pattern)] > thr.bgFC)
            }else{
                FALSE
            }
        })
        ...
        .ret <- NULL
        if(nrow(.i.pattern.res)>0){
            .i.pattern.res.idx <- which(.i.pattern.res$confirmed.aov)
            if(length(.i.pattern.res.idx)>0){
                .ret <- .i.pattern.res[.i.pattern.res.idx[1],]
            }
        }
        if(is.null(.ret)){
            .ret <- data.frame(Group=sprintf("P%s",paste(rep("0",ncol(.d.pattern)),collapse = "")),
                               pattern.cor=NA,
                               pattern.p=NA,
                               confirmed.aov=NA,stringsAsFactors = F)
        }
        rownames(.ret) <- x.rname
        return(.ret)
    ...
    gene.desc$geneSymbol <- g.GNAME[gene.desc$geneID]
    rownames(gene.desc) <- gene.desc$geneID
    gene.desc <- cbind(gene.desc,dat.to.sort.pattern)
    ##f.noNA <- !is.na(gene.desc[,"pattern.p"])
    ##gene.desc[f.noNA,"pattern.q"] <- p.adjust(gene.desc[f.noNA,"pattern.p"],method = "BH")
    pattern.gene.levels <- t(sapply(gene.desc$Group,function(.p.name){ 
        .ret <- .d.pattern[.p.name,]                
        if(max(.ret)==2){
            .ret[.ret==2] <- "hi"
            .ret[.ret==1] <- "int"
            .ret[.ret==0] <- "lo"
        ...
        names(.ret) <- sprintf("level.%s",names(.ret))
        return(.ret)
    }))
    ###colnames(pattern.gene.levels) <- sprintf("level.%s",colnames(.d.pattern))
    dat.to.sort.rename <- dat.to.sort
    colnames(dat.to.sort.rename) <- sprintf("mean.%s",colnames(dat.to.sort.rename))
    gene.desc <- cbind(gene.desc,pattern.gene.levels,dat.to.sort.rename)
    print(gene.desc[,grepl(pattern = "^level",x = colnames(gene.desc),perl = T)] %>% summary)

    gene.clusters <- gene.desc$Group
    names(gene.clusters) <- rownames(gene.desc)

    dat.pattern <- .d.pattern[unique(gene.desc$Group),]
    ...
    f.pattern.cor <- gene.desc[,"pattern.p"]<thr.cor.p
    ...
    f.dat.pattern.forAnn <- f.dat.pattern
    ...
    dat.pattern.plot.df <- melt(dat.pattern[names(gene.clusters.flt.count)[gene.clusters.flt.count>=5],,drop=F])
    ...
    n.flt.pattern <- sum(gene.clusters.flt.count>=5)
    pdf(sprintf("%s.gene.clusters.bin.pattern.pdf",out.prefix),
        width=if(n.flt.pattern>10) 8 else 4,
        height=n.flt.pattern*ifelse(n.flt.pattern>10,1,1.5)+1)
    p <- ggplot(dat.pattern.plot.df, aes(Var2, value)) + 
        geom_bar(stat = "identity",width=0.6) + 
        theme_bw(base_size = 12) + 
        facet_wrap(~ Var1,switch = "y",scales = "free",ncol=if(nrow(dat.pattern)>10) 2 else 1) + 
    ...
    print(p)
    dev.off()
    list(gene.clusters=gene.clusters,gene.desc=gene.desc,dat.plot=dat.plot,f.dat.pattern=f.dat.pattern,f.dat.pattern.forAnn=f.dat.pattern.forAnn)
}
...
if(args.method=="kmeans"){
    ...
    bin.res <- run.binarizedPatternAndPlot(out.prefix=out.prefix, dat.to.sort,aov.res=aov.res,col.sort=F,
                                           thr.bgFC=args.bgFC,nl = args.nlevels,thr.cor.p = args.thr.cor,n.cores = args.ncores)
...
f.col <- !(grepl("confirmed.aov|HSD.padj.min|HSD.padj.min.diff|HSD.padj.min.cmp|cluster.specific|is.clusterSpecific|cluster.lable|cluster.direction",colnames(out.df),perl=T) | colnames(out.df)=="q")
write.table(out.df[,f.col],
            file = sprintf("%s.aov.sig.geneGroup.txt",out.prefix),quote = F,sep = "\t",row.names = F)
if(args.method=="bin"){
    out.flt.df <- out.df[!is.na(out.df$pattern.p) & out.df$pattern.p<0.05,f.col]
    write.table(out.flt.df,
            file = sprintf("%s.aov.sig.geneGroup.flt.txt",out.prefix),quote = F,sep = "\t",row.names = F)
    write.table(out.df[bin.res$f.dat.pattern,f.col],
                file = sprintf("%s.aov.sig.geneGroup.majorPattern.txt",out.prefix),quote = F,sep = "\t",row.names = F)
}
...
run.annotation <- function(t.gene.clusters,dat.bg,out.dir,note="")
{
    ...
    for(i in sort(unique(t.gene.clusters)))
    {
        for(gsetCollection in names(annDB))
        {
            ...
            write.table(enrich.gset.df,sprintf("%s%s.G%s.%s.hyper.txt", .out.prefix,note,i,gsetCollection), 
                        quote = F,sep = "\t",row.names = F)
            write.table(enrich.gset.sig.df,sprintf("%s%s.G%s.%s.hyper.sig.txt", .out.prefix,note,i,gsetCollection), 
                        quote = F,sep = "\t",row.names = F)
...
if(!(args.notAnn)){
    if(args.method=="kmeans"){
        ...
        run.annotation(t.gene.clusters[bin.res$f.dat.pattern.forAnn],
                       dat.bg = Y.inGeneList[bin.res$f.dat.pattern.forAnn,],
                       sprintf("%s.ann",out.prefix),note=sprintf(".bin.pattern"))
        ###run.annotation(t.gene.clusters[bin.res$f.dat.pattern],
        ###               dat.bg = Y.inGeneList[bin.res$f.dat.pattern,],
        ###               sprintf("%s.ann",out.prefix),note=sprintf(".bin.pattern"))
        ### annotate hi/int expressed genes of each cluster
        for(.cls in names(majorClusterColor)){
            ##.gl <- "hi"
            ##.goilist <- rownames(bin.res$gene.desc)[bin.res$gene.desc[,sprintf("level.%s",.cls)]==.gl]
            for(.gl in c("hi","int","hi|int")){
                .goilist <- rownames(bin.res$gene.desc)[ grepl(.gl,bin.res$gene.desc[,sprintf("level.%s",.cls)],perl=T) ]
                .gl.str <- gsub("\\|","_",.gl)
                t.gene.clusters <- rep(sprintf("%s_%s",.cls,.gl.str),length(.goilist))
                names(t.gene.clusters) <- .goilist
                run.annotation(t.gene.clusters,
                           dat.bg = Y[.goilist,],
                           sprintf("%s.ann",out.prefix),note=sprintf(".bin.level"))
            }
        }
