...
parser$add_argument("-j", "--measure", type="character",
...
parser$add_argument("-q", "--useANOVA", action="store_true", default=FALSE, help="quick clustering and use diff genes for furthe clustering [default %(default)s]")
...
out.prefix <- sprintf("%s/%s.sc3",out.dir,sample.id)
...
if(!args.useANOVA){
    ...
    if(!is.null(args.nGene.list) && !is.null(args.nGene.list[["L1C1"]])){
        this.nKeep <- args.nGene.list[["L1C1"]]
    }else{
        this.nKeep <- nKeep
    }
    print("this.nKeep")
    print(this.nKeep)
    g.f <- head(names(Y.sd.sort),n=this.nKeep)
}else{
    ## by anova
    quick.clust <- run.quickClustBydynamicTreeCut(Y,useDyn=T,clustering.distance = "spearman", clustering.method = "complete")
    quick.aov.res <- runMultiGroupSpecificGeneTest(Y, grps = sprintf("G%s",quick.clust$my.clusters),
                                                  out.prefix,mod=NULL,FDR.THRESHOLD=0.05,FC.THRESHOLD=1,verbose=F,
                                                  n.cores=args.ncores,gid.mapping=NULL)
    #g.f <- head(rownames(quick.aov.res$aov.out.sig),n=nKeep)
    g.f <- rownames(quick.aov.res$aov.out.sig)
}
...
{
    ###g.f <- head(names(Y.sd.sort),n=nKeep)
    sc3.res <- runSC3Analysis(Y[g.f,iter.samples,drop=F],out.prefix,
                              sampleType,
                              sampleTypeColor,
                              do.log.scale=FALSE,
                              n.cores=args.ncores,
                              gid.mapping=g.GNAME,
                              data.forDE=Y.newRowname,
                              myseed=args.myseed,
                              clust.k = args.kmax,
                              frac.clustered = args.fracClustered,update.tsne = T)
                              ##data.forDE=NULL,
    if(is.null(sc3.res)){return(NULL)}
    #g.clust.res[[sprintf("L%s.%s",level,clusterName)]] <<- sc3.res
    g.clust.res[[sprintf("%s",clusterName)]] <<- sc3.res
    ##### recursively 
    best.k <- NULL
    if((level<args.iterMax)){
        ...
        if(is.null(best.k) && nrow(sc3.res$clustering.validation.df.best)>0){
            best.k <- sc3.res$clustering.validation.df.best[1,"k"]
        }
    }
    if(!is.null(best.k) && best.k>1){
        this.cluster <- sc3.res$ret.list[[best.k]]$cluster
        ...
        for(cls in sort(unique(this.cluster))){
            ...
            if(length(cls.sample)>10){
                ...
                cls.out.prefix <- sprintf("%s/%s/%s.SC3",out.dir,cls.aid,sample.id)
                dir.create(dirname(cls.out.prefix),showWarnings = F,recursive = T)
                print(cls.out.prefix)
                if(!args.useANOVA){
                    cls.Y.sd <- apply(Y[,cls.sample,drop=F], 1, sd)
                    cls.Y.sd.sort <- sort(cls.Y.sd,decreasing=TRUE)
                    if(!is.null(args.nGene.list) && !is.null(args.nGene.list[[cls.aid]])){
                        cls.nKeep <- args.nGene.list[[cls.aid]]
                    }else{
                        cls.nKeep <- nKeep
                    }
                    print("cls.nKeep")
                    print(cls.nKeep)
                    cls.g.f <- head(names(cls.Y.sd.sort),n=cls.nKeep)
                    print(str(cls.g.f))
                    cls.Y.sd.df <- data.frame(geneID=names(cls.Y.sd.sort),geneSymbol=g.GNAME[names(cls.Y.sd.sort)])
                    cls.Y.sd.df <- cbind(cls.Y.sd.df,cls.Y.sd.sort)
                    write.table(cls.Y.sd.df,sprintf("%s.cls.Y.sd.sort.txt",cls.out.prefix),row.names = F,sep = "\t",quote = F)
                }else{
                    cls.quick.clust <- run.quickClustBydynamicTreeCut(Y[,cls.sample,drop=F],useDyn=T,clustering.distance = "spearman", clustering.method = "complete")
                    cls.quick.aov.res <- runMultiGroupSpecificGeneTest(Y[,cls.sample,drop=F], grps = sprintf("G%s",cls.quick.clust$my.clusters),
                                                  out.prefix,mod=NULL,FDR.THRESHOLD=0.05,FC.THRESHOLD=1,verbose=F,
                                                  n.cores=args.ncores,gid.mapping=NULL)
                    cls.g.f <- rownames(cls.quick.aov.res$aov.out.sig)
                    ###cls.g.f <- head(rownames(cls.quick.aov.res$aov.out.sig),n=nKeep)
                }
...
do.it(g.f,
      out.prefix=sprintf("%s/L1C1/%s.SC3",out.dir,sample.id),
...
get.iter.cluster <- function(g.clust.res)
{
    ...
    for(aid in names(g.clust.res)){
        ...
        if(!is.null(aid.best.k) && aid.best.k>1){
            aid.cluster <- g.clust.res[[aid]][["ret.list"]][[aid.best.k]][["cluster"]]
...
if(!is.null(iter.cluster.df))
{
    ...
    if(nrow(iter.gene.table$gene.table)>10){
        iter.tsne.res.update <- runTSNEAnalysis(Y[rownames(iter.gene.table$gene.table),iter.cluster.df$sample,drop=F],sprintf("%s/%s.iter.tsne.update.00",out.dir,sample.id),
...
save.image(file = sprintf("%s/%s.iter.RData",out.dir,sample.id))
